...
class Ebizmarts_MailChimp_Model_Api_BatchesTest extends PHPUnit_Framework_TestCase
{
        ...
        return array(
            ...
            '_links' => array(
                array(
                    ...
                    'method' => 'GET',
                    'targetSchema' => 'https://us13.api.mailchimp.com/schema/3.0/Definitions/Batches/
    ...
    protected function getPromoCodeArray()
    {
        return array(
            array(
                ...
                'body' => '{"id":"44","title":"testrule","description":"testrule","amount":0.05,"type":"percentage",
                "target":"total","enabled":true}'
            ),
            array(
                'method' => 'POST',
    ...
    protected function getOrderArray()
    {
        return array(
            array(
                ...
                "updated_at_foreign":"2018-01-16 14:26:55","lines":[{"id":"1","product_id":"337",
                "product_variant_id":"337","quantity":1,"price":"295.0000","discount":0}],"customer":{"id":"137",
                "email_address":"santiago+testtest@ebizmarts.com","opt_in_status":false,"first_name":"Santiago",
                "last_name":"Paragarino","address":{"address1":"address","city":"city","province":"Alabama",
                "province_code":"AL","postal_code":"123456","country":"United States","country_code":"US"},
                "orders_count":1,"total_spent":600},"order_url":"http:\/\/127.0.0.1\/mcmagento-1937\/sales\/order\
                /view\/order_id\/195\/?___store=default","billing_address":{"address1":"address","city":"city",
                "province":"Alabama","province_code":"AL","postal_code":"123456","country":"United States",
                "country_code":"US","name":"Santiago Paragarino"},"shipping_address":{"name":"Santiago Paragarino",
                "address1":"address","city":"city","province":"Alabama","province_code":"AL","postal_code":"123456",
    ...
    protected function getCartArray()
    {
        return array(
            array(
                ...
                "checkout_url":"http:\/\/127.0.0.1\/mcmagento-1937\/mailchimp\/cart\
                /loadquote?id=681&token=0eaf8c240502056a62b9194dd2ed0859","currency_code":"USD","order_total":"10.0000",
                "tax_total":0,"lines":[{"id":"1","product_id":"906","product_variant_id":"906",
    ...
    protected function getProductArray()
    {
        return array(
            array(
                ...
                "url":"http:\/\/127.0.0.1\/mcmagento-1937\/aviator-sunglasses.html","image_url":
                "http:\/\/127.0.0.1\/mcmagento-1937\/media\/catalog\/product\/cache\/1\/image\/265x\
                /9df78eab33525d08d6e5fb8d27136e95\/a\/c\/ace000a_1.jpg","published_at_foreign":"",
                "description":"Gunmetal frame with crystal gradient polycarbonate lenses in grey. ","type":"Eyewear",
                "vendor":"Eyewear","handle":"","variants":[{"id":"337","title":"Aviator Sunglasses",
                "url":"http:\/\/127.0.0.1\/mcmagento-1937\/aviator-sunglasses.html",
                "image_url":"http:\/\/127.0.0.1\/mcmagento-1937\/media\/catalog\/product\/cache\/1\/image\/265x\
                /9df78eab33525d08d6e5fb8d27136e95\/a\/c\/ace000a_1.jpg","published_at_foreign":"","sku":"ace000",
                "price":295,"inventory_quantity":6,"backorders":"0","visibility":"Catalog, Search"}]}'
            ),
            array(
                'method' => 'POST',
                ...
                'body' => '{"id":"906","title":"test Prod","url":"http:\/\/127.0.0.1\/mcmagento-1937\/test-prod.html",
                "published_at_foreign":"","description":"Test","type":"Default Category","vendor":"Default Category",
                "handle":"","variants":[{"id":"906","title":"test Prod","url":"http:\/\/127.0.0.1\/mcmagento-1937\
                /test-prod.html","published_at_foreign":"","sku":"testprod","price":10,"inventory_quantity":1000,
                "backorders":"0","visibility":"Catalog, Search"}]}'
            )
        );
    }
    ...
    protected function getCustomerArray()
    {
        return array(
            array(
                ...
                "last_name":"Paragarino","opt_in_status":false,"orders_count":1,
                "total_spent":300,"address":{"address1":"address","city":"city",
                "province":"Alabama","province_code":"AL","postal_code":"123456","country":"United States",
                "country_code":"US"}}'
            )
        );
    ...
    protected function getSubscriberArray()
    {
        return array(array(
            ...
            'body' => '{"email_address":"santiago+testing3@ebizmarts.com","merge_fields":{"WEBSITE":"1","STOREID":"1",
            "STORECODE":"default","CGROUP":"NOT LOGGED IN","CREATEDAT":"2018-02-05T12:59:01-08:00"},
            "status_if_new":"subscribed","language":"en_US"}'
        ));
    }
    ...
    public function testHandleEcommerceBatches()
    {
        ...
        $apiBatchesMock = $this->apiBatchesMock
            ...
            ->setMethods(
                array('getHelper', '_getResults', '_sendEcommerceBatch', 'addSyncValueToArray',
                'handleSyncingValue', 'getStores')
            )
            ->getMock();
        ...
        $apiBatchesMock->expects($this->once())->method('addSyncValueToArray')->with($storeId, $syncedArray)
    ...
    public function testSendEcommerceBatch()
    {
        ...
        $apiBatchesMock->expects($this->once())->method('getHelper')->willReturn($helperMock);
        $helperMock->expects($this->once())->method('getMCStoreId')->with($magentoStoreId)
        ...
        $apiCustomersMock->expects($this->once())->method('createBatchJson')->with($mailchimpStoreId, $magentoStoreId)
            ->willReturn($customerArray);

        $apiBatchesMock->expects($this->once())->method('getApiProducts')->willReturn($apiProductsMock);
        $apiProductsMock->expects($this->once())->method('createBatchJson')->with($mailchimpStoreId, $magentoStoreId)
            ->willReturn($productsArray);

        $apiBatchesMock->expects($this->once())->method('getApiCarts')->willReturn($apiCartsMock);
        $apiCartsMock->expects($this->once())->method('createBatchJson')->with($mailchimpStoreId, $magentoStoreId)
            ->willReturn($cartsArray);

        $apiBatchesMock->expects($this->once())->method('getApiOrders')->willReturn($apiOrdersMock);
        $apiOrdersMock->expects($this->once())->method('createBatchJson')->with($mailchimpStoreId, $magentoStoreId)
            ->willReturn($ordersArray);

        $apiBatchesMock->expects($this->once())->method('getApiPromoRules')->willReturn($apiPromoRulesMock);
        $apiPromoRulesMock->expects($this->once())->method('createBatchJson')->with($mailchimpStoreId, $magentoStoreId)
            ->willReturn($promoRulesArray);

        $apiBatchesMock->expects($this->once())->method('getApiPromoCodes')->willReturn($apiPromoCodesMock);
        $apiPromoCodesMock->expects($this->once())->method('createBatchJson')->with($mailchimpStoreId, $magentoStoreId)
        ...
        $apiBatchesMock->expects($this->once())->method('markItemsAsSent')
        ...
        $helperMock->expects($this->once())->method('getEcommMinSyncDateFlag')->with($magentoStoreId)
    ...
    public function testGetResults()
    {
        ...
        $helperMock->expects($this->once())->method('getMCStoreId')
            ->with($magentoStoreId)->willReturn($mailchimpStoreId);
        ...
        $syncBatchesCollectionMock->expects($this->at(0))->method('addFieldToFilter')
            ->with('status', array('eq' => 'pending'))->willReturnSelf();
        $syncBatchesCollectionMock->expects($this->at(1))->method('addFieldToFilter')
            ->with('store_id', array('eq' => $mailchimpStoreId))->willReturnSelf();
        ...
        $syncBatchesCollectionMock->expects($this->once())->method("getIterator")
            ->willReturn(new ArrayIterator($syncBatches));

        $apiBatchesMock->expects($this->once())->method('getBatchResponse')
            ->with($batchId, $magentoStoreId)->willReturn($files);
        $apiBatchesMock->expects($this->once())->method('processEachResponseFile')
            ->with($files, $batchId, $mailchimpStoreId);

        $apiBatchesMock->expects($this->once())->method('getMagentoBaseDir')->willReturn($magentoBaseDir);
        $apiBatchesMock->expects($this->once())->method('batchDirExists')
            ->with($magentoBaseDir, $batchId)->willReturn(true);
    ...
    public function testSendStoreSubscriberBatch()
    {
        ...
        $apiSubscribersMock->expects($this->once())->method('createBatchJson')->with($listId, $storeId, $limit)
            ->willReturn($subscribersArray);

        $helperMock->expects($this->once())->method('getApi')->with($storeId)->willReturn($apiMock);

        $apiMock->expects($this->once())->method('getBatchOperation')->willReturn($apiBatchOperationMock);

        $apiBatchOperationMock->expects($this->once())->method('add')->with($subscribersJson)
    ...
    public function testHandleSyncingValue()
    {
        ...
        $helperMock = $this->getMockBuilder(Ebizmarts_MailChimp_Helper_Data::class)
            ->disableOriginalConstructor()
            ->setMethods(
                array('isEcomSyncDataEnabled', 'getApi', 'getMailChimpScopeByStoreId', 'getConfig',
        ...
        $helperMock->expects($this->once())->method('isEcomSyncDataEnabled')->with($magentoStoreId)->willReturn(true);
        $helperMock->expects($this->once())->method('getApi')->with($magentoStoreId)->willReturn($apiMock);
        $helperMock->expects($this->once())->method('getDateSyncFinishByMailChimpStoreId')->with($mailchimpStoreId)
            ->willReturn(null);
        ...
        $helperMock->expects($this->once())->method('getMailChimpScopeByStoreId')->with($magentoStoreId)
            ->willReturn($scopeToEdit);
        $helperMock->expects($this->once())->method('getConfig')->willReturn($configMock);

        $configMock->expects($this->once())->method('deleteConfig')
            ->with(
                Ebizmarts_MailChimp_Model_Config::
                GENERAL_MCISSYNCING, $scopeToEdit['scope'], $scopeToEdit['scope_id']
            );
