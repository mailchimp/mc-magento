...
class Ebizmarts_MailChimp_Adminhtml_MailchimperrorsControllerTest extends PHPUnit_Framework_TestCase
{
     ...
     */
    private $_mailchimperrorsController;
    ...
    public function testDownloadresponseAction()
    {
        ...
        "title":"Invalid Resource","status":400,"detail":"The resource submitted could not be validated.
         For field-specific details, see the \'errors\' array.","instance":"","errors":[{"field":"email_address",
         "message":"This email address looks fake or invalid. Please enter a real email address."}]}';
        $items = array($item);
        ...
        $mailchimperrorsControllerMock->expects($this->once())->method('getMailchimperrorsModel')
        ...
        $apiBatchesMock->expects($this->once())->method('getBatchResponse')
            ->with($batchId, $storeId)->willReturn($files);

        $mailchimperrorsControllerMock->expects($this->once())->method('getFileContent')
            ->with($file)->willReturn($items);
        ...
        $apiBatchesMock->expects($this->once())->method('batchDirExists')->with($magentoBaseDir, $batchId)
            ->willReturn(true);
