...
class Mandrill_Templates
{
     ...
     *         - labels[] string a single label
     *     - code string the full HTML code of the template, with mc:edit attributes marking the editable elements
     *      - draft version
     ...
     *     - publish_code string the full HTML code of the template, with mc:edit attributes marking the editable
     ...
     *     - published_at string the date and time the template was last published as a UTC string
     *      in YYYY-MM-DD HH:MM:SS format, or null if it has not been published
     *     - created_at string the date and time the template was first created as a UTC string
     *          in YYYY-MM-DD HH:MM:SS format
     *     - updated_at string the date and time the template was last modified as a UTC string
     *          in YYYY-MM-DD HH:MM:SS format
     */
    public function add($name, $fromEmail = null, $fromName = null, $subject = null, $code = null, $text = null,
                        $publish = true, $labels = array())
    {
        $_params = array("name" => $name, "from_email" => $fromEmail, "from_name" => $fromName, "subject" => $subject,
            "code" => $code, "text" => $text, "publish" => $publish, "labels" => $labels);
     ...
     *     - created_at string the date and time the template was first created as a UTC string
     *      in YYYY-MM-DD HH:MM:SS format
     *     - updated_at string the date and time the template was last modified as a UTC string in YYYY-MM-DD HH:MM:SS
     *      format
     */
     ...
     *         - labels array the list of labels applied to the template
     *             - labels[] string a single label
     *         - code string the full HTML code of the template, with mc:edit attributes marking the editable
     *          elements - draft version
     *         - subject string the subject line of the template, if provided - draft version
     *         - from_email string the default sender address for the template, if provided - draft version
     *         - from_name string the default sender from name for the template, if provided - draft version
     *         - text string the default text part of messages sent with the template, if provided - draft version
     *         - publish_name string the same as the template name - kept as a separate field
     ...
     * @param  array  $templateContent an array of template content to render.  Each item in the array should be a
     * struct with two keys - name: the name of the content block to set the content for, and content: the actual
     * content to put into the block
     * - template_content[] struct the injection of a single piece of content into
     * a single editable region - name string the name of the mc:edit editable region
     * to inject into - content string the content to inject
     * @param  array  $mergeVars       optional merge variables to use for injecting merge field content.
     *                                  If this is not provided, no merge fields will be replaced.
     *                                  - merge_vars[] struct a single merge variable - name string the merge
     *                                  variable's name. Merge variable names are
    ...
    {
        $_params = array("template_name" => $templateName, "template_content" => $templateContent,
