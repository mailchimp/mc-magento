...
class MailChimp_EcommercePromoRules extends MailChimp_Abstract
{
     ...
     * @param string    $description        The description of a promotion restricted to UTF-8 characters with max
     *                                      length 255.
     * @param float     $amount             The amount of the promo code discount. If ‘type’ is ‘fixed’,
     *                                      the amount is treated as a monetary value.
     *                                      If ‘type’ is ‘percentage’, amount must be a decimal value
     *                                      between 0.0 and 1.0, inclusive.
     * @param string    $type               Type of discount. For free shipping set type to fixed. Possible Values:
     *                                      'fixed', 'percentage'
     ...
     * @param null      $endsAt            The date and time when the promotion ends. Must be after starts_at and in
     *                                      ISO 8601 format.
     ...
     */
    public function add($storeId, $promoRuleId, $description, $amount, $type, $target, $startsAt = null,
                        $endsAt = null, $enabled = null, $createdAtForeign = null, $updatedAtForeign = null,
                        $title = null)
    {
        $_params = array('id' => $promoRuleId, 'description' => $description, 'amount' => $amount, 'type' => $type,
            'target' => $target);
        ...
        return $this->_master->call(
            'ecommerce/stores/' . $storeId . '/promo-rules', $_params,
     ...
     * @param string    $storeId                The MailChimp store id.
     * @param int       $promoRuleId            The id for the promo rule of a store.
     * @param null      $title                  The title that will show up in promotion campaign. Restricted to UTF-8
     *                                          characters with max length 100.
     * @param string    $description            The description of a promotion restricted to UTF-8 characters with max
     *                                          length 255.
     * @param float     $amount                 The amount of the promo code discount. If ‘type’ is ‘fixed’,
     *                                          the amount is treated as a monetary value.
     ...
     * @return mixed
     */
    public function modify($storeId, $promoRuleId, $description, $amount, $type, $target,
                           $startsAt = null, $endsAt = null, $enabled = null, $createdAtForeign = null,
                           $updatedAtForeign = null, $title = null)
    {
        $_params = array();
        if ($title) {
            $_params['title'] = $title;
        ...
        return $this->_master->call(
            'ecommerce/stores/' . $storeId . '/promo-rules/' . $promoRuleId, $_params,
