...
class Ebizmarts_MailChimp_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public function deleteLocalMCStoreData($mailchimpStoreId, $scopeId, $scope = null)
    {
        ...
        $config->deleteConfig($this->ecommerceMcJsUrl(), $scope, $scopeId);
        ...
        $config->deleteConfig(
            Ebizmarts_MailChimp_Model_Config::ECOMMERCE_SYNC_DATE .
            "_$mailchimpStoreId",
            'default',
            0
        );
    ...
    protected function isRequestLogEnabled()
    {
        ...
        if ($logConfig == $this->requestLog() || $logConfig == $this->sourceLogBoth()) {
            $logEnabled = true;
        }
    ...
    protected function isErrorLogEnabled()
    {
        $logEnabled = false;
        $logConfig = $this->getLogsEnabled();
        if ($logConfig == $this->errorLog() || $logConfig == $this->sourceLogBoth()) {
            $logEnabled = true;
        }
    ...
    /**
     * Reset error messages from Products, Subscribers, Customers,
     * Orders, Quotes and set them to be sent again for given scope.
     *
     * @param $scopeId
     * @param $scope
     */
    ...
    public function getApi($scopeId, $scope = null)
    {
        ...
        if ($apiKey != null && $apiKey != "") {
            $api = new Ebizmarts_MailChimp(
                ...
                null,
                'Mailchimp4Magento' .
                (string)$this->getConfig()->getNode(
                    'modules/Ebizmarts_MailChimp/version'
                )
            );
        } else {
    ...
    {
        if ($this->getMCStoreId($scopeId, $scope) && $this->getIfConfigExistsForScope(
    ...
    public function getMCJs()
    {
        ...
        if ($this->isEcomSyncDataEnabled($storeId)) {
            $currentUrl = $this->getConfigValueForScope($this->ecommerceMcJsUrl(), $storeId);
    ...
    {
        $initialTime = time();
        $migrateFromHundredFifteen = $this->getConfigValueForScope(
            Ebizmarts_MailChimp_Model_Config::GENERAL_MIGRATE_FROM_115,
            0,
            'default'
        );
        $migrateFromOneHundredSixteen = $this->getConfigValueForScope(
            Ebizmarts_MailChimp_Model_Config::GENERAL_MIGRATE_FROM_116,
            0,
            'default'
        );
        $migrateFromOneThousandSixtyFour = $this->getConfigValueForScope(
        ...
        if ($migrateFromHundredFifteen) {
            $this->_migrateFrom115($initialTime);
        } elseif ($migrateFromOneHundredSixteen && !$this->timePassed($initialTime)) {
            $this->_migrateFrom116($initialTime);
        } elseif ($migrateFromOneThousandSixtyFour && !$this->timePassed($initialTime)) {
    ...
    protected function _migrateCustomersFrom115($mailchimpStoreId, $initialTime)
    {
        try {
            ...
            $attribute = Mage::getModel('customer/attribute')->loadByCode(
                $entityType,
    ...
    protected function _migrateCartsFrom115($mailchimpStoreId, $initialTime)
    {
        try {
            ...
            if (isset($quoteFields['mailchimp_sync_delta'])) {
                ...
                $quoteCollection->getSelect()->where(
                    "m4m.mailchimp_sync_delta IS NULL AND main_table.mailchimp_sync_delta > '0000-00-00 00:00:00'"
                );
    ...
    public function migrationFinished()
    {
        ...
        if (!$migrateFromHundredFifteen && !$migrateFromOneHundredSixteen && !$migrateFromOneThousandSixtyFour) {
        ...
        try {
            ...
            try {
                ...
                if ($e->getMailchimpDetails() == $textToCompare) {
                    $errorMessage =
                        'Your store could not be accessed by MailChimp\'s Api. Please confirm the URL: ' .
                        $hookUrl .
    ...
    {
        return $this->isNotDefaultScope($config) && ($this->isIncorrectScope($config, $scope) ||
    ...
    public function isCheckoutSubscribeEnabled($scopeId, $scope = 'stores')
    {
        return ($this->isSubscriptionEnabled($scopeId, $scope)
            && $this->checkoutSubscriberIsNotDisabled($scopeId, $scope));
    }
    ...
    protected function isMissingCustomerLowerThanId($itemId, $storeId)
    {
        ...
        Mage::getModel('mailchimp/api_customers')->joinMailchimpSyncDataWithoutWhere(
            $customerCollection,
            $mailchimpStoreId
        );
    ...
    protected function isMissingOrderLowerThanId($itemId, $storeId)
    {
        ...
        Mage::getModel('mailchimp/api_orders')->joinMailchimpSyncDataWithoutWhere(
            $orderCollection,
            $mailchimpStoreId
        );
    ...
    protected function isMissingQuoteLowerThanId($itemId, $storeId)
    {
        ...
        Mage::getModel('mailchimp/api_carts')->joinMailchimpSyncDataWithoutWhere(
            $quoteCollection,
            $mailchimpStoreId
        );
    ...
    protected function areJsUrlAndListScopesEqual($storeId)
    {
        ...
        $realScopeJs = $this->getRealScopeForConfig($this->ecommerceMcJsUrl(), $storeId);
        if ($realScopeList &&
            $realScopeJs &&
            $realScopeList['scope'] == $realScopeJs['scope'] &&
            $realScopeList['scope_id'] == $realScopeJs['scope_id']
        ) {
            $scopesMatch = true;
        }
        return $scopesMatch;
    ...
    public function changeStoreNameIfRequired($groupStoreName, $scopeId, $scope)
    {
        $mailchimpStoreId = $this->getMCStoreId($scopeId, $scope);
        if ($mailchimpStoreId) {
            $realScope = $this->getRealScopeForConfig(
                Ebizmarts_MailChimp_Model_Config::GENERAL_MCSTOREID,
                $scopeId,
                $scope
            );
    ...
    protected function _lang2MCLanguage($languageCode = '')
    {
        ...
        if (in_array($languageCode, self::$mailchimpLanguages)) {
            ...
            $langIsoTwo = substr($languageCode, 0, 2);
            if (in_array($langIsoTwo, self::$mailchimpLanguages)) {
                $mailchimpLanguage = $langIsoTwo;
            }
        }
    ...
    {
        return Ebizmarts_MailChimp_Model_System_Config_Source_Log::ERROR_LOG;
    }

    /**
     * @return int
     */
    protected function sourceLogBoth()
    {
        return Ebizmarts_MailChimp_Model_System_Config_Source_Log::BOTH;
    }
    ...
    {
        return Ebizmarts_MailChimp_Model_System_Config_Source_Log::REQUEST_LOG;
    }

    /**
     * @return string
     */
    protected function ecommerceMcJsUrl()
    {
        return Ebizmarts_MailChimp_Model_Config::ECOMMERCE_MC_JS_URL;
    }
     ...
     * @return bool
     */
    protected function checkoutSubscriberIsNotDisabled($scopeId, $scope)
    {
        return $this->getCheckoutSubscribeValue($scopeId, $scope) != $this->isDisabled();
    }
    ...
    {
        return Ebizmarts_MailChimp_Model_System_Config_Source_Checkoutsubscribe::DISABLED;
    }
}
