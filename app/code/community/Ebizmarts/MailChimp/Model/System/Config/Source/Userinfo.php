...
class Ebizmarts_MailChimp_Model_System_Config_Source_Userinfo
{
    ...
    public function __construct()
    {
        ...
        if ((!is_array($this->_accountDetails) || isset($this->_accountDetails['status'])) &&
            Mage::getStoreConfig(Ebizmarts_MailChimp_Model_Config::MANDRILL_APIKEY, $storeId)) {
            $api = new Mandrill_Message(
                Mage::getStoreConfig(Ebizmarts_MailChimp_Model_Config::MANDRILL_APIKEY, $storeId)
            );
            try {
            ...
            if (!isset($this->_accountDetails['status'])) {
                return array(
                    array('value' => 0, 'label' => $helper->__(
                        "<strong>Username</strong>: %s %s", $this
                        ->_accountDetails["username"], "<small>used for SMTP authentication</small>"
                    )),

                    array('value' => 1, 'label' => $helper->__(
                        '<strong>Reputation</strong>: %s %s', $this
                        ->_accountDetails['reputation'],
                        "<small>scale from 0 to 100, with 75 generally being a \"good\" reputation</small>"
                    )),

                    array('value' => 2, 'label' => $helper->__(
                        '<strong>Hourly Quota</strong>: %s %s', $this
                        ->_accountDetails['hourly_quota'],
