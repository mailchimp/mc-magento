...
class Ebizmarts_MailChimp_Model_Api_Orders
{
    ...
    public function GeneratePOSTPayload($order, $mailchimpStoreId, $magentoStoreId)
    {
        ...
        if (!$lines['itemsCount']) {
            unset($data['lines']);
            $helper->logWarning(
                "Ignoring e-commerce batch for store $magentoStoreId: order ID {$orderId} "
                . (isset($data['campaign_id']) ? "campaign ID " . $data['campaign_id'].' ' : '')
                . "currency {$data['currency_code']} total {$data['order_total']} "
                . "tax {$data['tax_total']} discount {$data['discount_total']} "
                . "shipping {$data['shipping_total']}: no supported products"
            );
    ...
    protected function _getPayloadDataLines($order, $mailchimpStoreId, $magentoStoreId)
    {
        ...
        foreach ($items as $item) {
            ...
            if (!$isProductEnabled || ($productSyncData->getMailchimpSyncDelta() && $productSyncError == '')) {
                ...
                $helper->logDebug(
                    " => Store $magentoStoreId batch for order ID {$orderId} "
                    . "item $itemCount product ID $productId variant $variant quantity "
                    . (int)$item->getQtyOrdered() . " price " . $item->getPrice()
                    . " discount " . abs($item->getDiscountAmount())
                );
