...
class Ebizmarts_MailChimp_Model_Api_PromoRules
{
    ...
    {
        $batchArray = array();
        $this->_batchId = 'storeid-' . $magentoStoreId . '_' . Ebizmarts_MailChimp_Model_Config::IS_PROMO_RULE .
            '_' . $this->getMailChimpHelper()->getDateMicrotime();
    ...
    public function getNewPromoRule($ruleId, $mailchimpStoreId, $magentoStoreId)
    {
        ...
        try {
            ...
            if (!empty($ruleData)) {
                ...
                $promoData['operation_id'] = 'storeid-' . $magentoStoreId . '_' . Ebizmarts_MailChimp_Model_Config::
                    IS_PROMO_RULE . '_' . $helper->getDateMicrotime() . '_' . $ruleId;
                ...
                $this->_updateSyncData($ruleId, $mailchimpStoreId, Mage::getSingleton('core/date'), $error);
            }
        } catch (Exception $e) {
    ...
    protected function makeModifiedAndDeletedPromoRulesCollection($mailchimpStoreId)
    {
        ...
        $deletedPromoRules->getSelect()->where(
            "mailchimp_store_id = '" . $mailchimpStoreId . "' AND type = '" .
            Ebizmarts_MailChimp_Model_Config::IS_PROMO_RULE . "' AND (mailchimp_sync_modified = 
            1 OR mailchimp_sync_deleted = 1)"
        );
     ...
     */
    protected function _updateSyncData($ruleId, $mailchimpStoreId, $syncDelta = null, $syncError = null,
                                       $syncModified = 0, $syncDeleted = null, $saveOnlyIfexists = false,
    ...
    protected function deletePromoRuleSyncData($ruleId, $mailchimpStoreId)
    {
        $ruleSyncDataItem = $this->getMailChimpHelper()->getEcommerceSyncDataItem(
            $ruleId,
            Ebizmarts_MailChimp_Model_Config::IS_PROMO_RULE, $mailchimpStoreId
        );
    ...
    protected function _setModified($ruleId)
    {
        ...
        foreach ($promoRules as $promoRule) {
            ...
            $this->_updateSyncData(
                $ruleId, $mailchimpStoreId, null, null, 1,
                null, true, false
            );
        }
    }
    ...
    protected function _setDeleted($ruleId)
    {
        ...
        foreach ($promoRules as $promoRule) {
            $mailchimpStoreId = $promoRule->getMailchimpStoreId();
            $this->_updateSyncData(
                $ruleId, $mailchimpStoreId, null, null, null,
                1, true, false
            );
        }
    }
