...
class Ebizmarts_MailChimp_Model_Api_PromoCodes
{
    ...
    {
        $batchArray = array();
        $this->_batchId = 'storeid-' . $magentoStoreId . '_' . Ebizmarts_MailChimp_Model_Config::IS_PROMO_CODE .
            '_' . Mage::helper('mailchimp')->getDateMicrotime();
    ...
    protected function _getDeletedPromoCodes($mailchimpStoreId)
    {
        ...
        foreach ($deletedPromoCodes as $promoCode) {
            ...
            $batchArray[$counter]['path'] = '/ecommerce/stores/' . $mailchimpStoreId . '/promo-rules/' . $promoRuleId .
    ...
    protected function _getNewPromoCodes($mailchimpStoreId, $magentoStoreId)
    {
        ...
        $newPromoCodes->getSelect()->where(
            "m4m.mailchimp_sync_delta IS NULL AND website.website_id = " .
            $websiteId . " AND ( " . $autoGeneratedCondition . " OR " . $notAutoGeneratedCondition . ")"
        );
        ...
        foreach ($newPromoCodes as $promoCode) {
            ...
            try {
                ...
                if (!$promoRuleSyncData->getId() || $promoRuleSyncData->getMailchimpSyncDelta() < $helper->
                    getEcommMinSyncDateFlag($magentoStoreId)) {
                ...
                if (!empty($promoCodeData)) {
                    ...
                    $batchArray[$counter]['path'] = '/ecommerce/stores/' . $mailchimpStoreId . '/promo-rules/' .
                        $ruleId . '/promo-codes';
                    $batchArray[$counter]['operation_id'] = $this->_batchId . '_' . $codeId;
                    $batchArray[$counter]['body'] = $promoCodeJson;

                    $this->_updateSyncData(
                        $codeId, $mailchimpStoreId, null, null, null,
                        null, $promoCode->getToken()
                    );
                    $counter++;
                } else {
    ...
    protected function makeDeletedPromoCodesCollection($mailchimpStoreId)
    {
        ...
        $deletedPromoCodes->getSelect()->where(
            ...
            Ebizmarts_MailChimp_Model_Config::IS_PROMO_CODE . "' AND mailchimp_sync_deleted = 1"
        );
     ...
     */
    protected function _updateSyncData($codeId, $mailchimpStoreId, $syncDelta = null, $syncError = null,
                                       $syncModified = 0, $syncDeleted = null, $token = null, $saveOnlyIfexists = false,
                                       $deletedRelatedId = null, $allowBatchRemoval = true)
    {
    ...
    protected function getRedemptionUrl($promoCode, $magentoStoreId)
    {
        ...
        $url = Mage::getModel('core/url')->setStore($magentoStoreId)->getUrl(
            ...
            array('_nosid' => true, '_secure' => true, 'coupon_id' =>$promoCode->getCouponId(),
            'coupon_token' => $token)
        ) . 'mailchimp/cart/loadcoupon?coupon_id=' . $promoCode->getCouponId() .
            '&coupon_token=' . $token;
        return $url;
    }
    ...
    public function deletePromoCodeSyncData($promoCodeId, $mailchimpStoreId)
    {
        $promoCodeSyncDataItem = $this->getMailChimpHelper()->getEcommerceSyncDataItem(
            $promoCodeId,
            Ebizmarts_MailChimp_Model_Config::IS_PROMO_CODE, $mailchimpStoreId
        );
        $promoCodeSyncDataItem->delete();
    }
