...
class Ebizmarts_MailChimp_Model_Api_PromoCodes
{
    ...
    {
        $batchArray = array();
        $dateMicrotime = Mage::helper('mailchimp')->getDateMicrotime();
        $this->_batchId = 'storeid-' . $magentoStoreId . '_' . $this->isPromCode() . '_' . $dateMicrotime;
    ...
    protected function _getDeletedPromoCodes($mailchimpStoreId)
    {
        ...
        foreach ($deletedPromoCodes as $promoCode) {
            ...
            $ecomS = '/ecommerce/stores/';
            $promR = '/promo-rules/';
            $promC = '/promo-codes/';
            $batchArray[$counter]['path'] = $ecomS.$mailchimpStoreId. $promR .$promoRuleId. $promC .$promoCodeId;
    ...
    protected function _getNewPromoCodes($mailchimpStoreId, $magentoStoreId)
    {
        ...
        $autoGeneCondit = "salesrule.use_auto_generation = 1 AND main_table.is_primary IS NULL";
        $noAutoGeneCondit = "salesrule.use_auto_generation = 0 AND main_table.is_primary = 1";
        $mailSync = "m4m.mailchimp_sync_delta IS NULL AND website.website_id = ";
        $newPromoCodes->getSelect()->where($mailSync.$websiteId." AND ( ".$autoGeneCondit." OR ".$noAutoGeneCondit.")");
        ...
        foreach ($newPromoCodes as $promoCode) {
            ...
            try {
                $promoRuleSyncData = $this->getMailChimpHelper()->getEcommerceSyncDataItem($ruleId, $this->isPromoRule(), $mailchimpStoreId);
                ...
                if (!empty($promoCodeData)) {
                    ...
                    $strEcSto = '/ecommerce/stores/';
                    $strPromRul = '/promo-rules/';
                    $strPromCode = '/promo-codes';
                    $batchArray[$counter]['path'] = $strEcSto.$mailchimpStoreId.$strPromRul.$ruleId.$strPromCode;
    ...
    public function makePromoCodesCollection($magentoStoreId)
    {
        ...
        $helper->addResendFilter($collection, $magentoStoreId, $this->isPromCode());
    ...
    protected function makeDeletedPromoCodesCollection($mailchimpStoreId)
    {
        ...
        $maChimpSyncDel = "' AND mailchimp_sync_deleted = 1";
        $andType = "' AND type = '";
        $mCStore = "mailchimp_store_id = '";
        $deletedPromoCodes->getSelect()->where($mCStore.$mailchimpStoreId.$andType.$this->isPromCode().$maChimpSyncDel);
    ...
    public function joinMailchimpSyncDataWithoutWhere($collection, $mailchimpStoreId)
    {
        ...
        $collection->getSelect()->joinLeft(
            ...
            sprintf($joinCondition, $this->isPromCode(), $mailchimpStoreId), array(
                "m4m.related_id",
                "m4m.type",
    ...
    protected function getRedemptionUrl($promoCode, $magentoStoreId)
    {
        ...
        $strLoadCoupon = 'mailchimp/cart/loadcoupon';
        $model = Mage::getModel('core/url')->setStore($magentoStoreId)->getUrl($strLoadCoupon, array('_nosid' => true, '_secure' => true, 'coupon_id' => $promoCode->getCouponId(), 'coupon_token' => $token));
        $url = $model . 'mailchimp/cart/loadcoupon?coupon_id=' . $promoCode->getCouponId() . '&coupon_token=' . $token;
        return $url;
    }
    ...
    {
        $helper = $this->getMailChimpHelper();
        $promoCodes = $helper->getAllEcommerceSyncDataItemsPerId($codeId, $this->isPromCode());
    ...
    public function deletePromoCodesSyncDataByRule($promoRule)
    {
        ...
        foreach ($promoCodeIds as $promoCodeId) {
            $promoCodeSyncDataItems = $this->getMailChimpHelper()->getAllEcommerceSyncDataItemsPerId($promoCodeId, $this->isPromCode());
    ...
    {
        $promoCodeSyncDataItem = $this->getMailChimpHelper()->getEcommerceSyncDataItem($promoCodeId, $this->isPromCode(), $mailchimpStoreId);
        $promoCodeSyncDataItem->delete();
    }
    ...
    protected function getPromoCodesForRule($promoRuleId)
    {
        $promoCodes = array();
        $helper = $this->getMailChimpHelper();
        $promoRules = $helper->getAllEcommerceSyncDataItemsPerId($promoRuleId, $this->isPromoRule());
