...
class Ebizmarts_MailChimp_Model_Api_PromoCodes
{
    ...
    public function createBatchJson($mailchimpStoreId, $magentoStoreId)
    {
        ...
        $this->_batchId = 'storeid-' . $magentoStoreId . '_' . $this->isPromoCode() . '_' . $dateMicrotime;
    ...
    protected function _getDeletedPromoCodes($mailchimpStoreId)
    {
        ...
        foreach ($deletedPromoCodes as $promoCode) {
            ...
            $ecommerceS = '/ecommerce/stores/';
            $promoR = '/promo-rules/';
            $promoC = '/promo-codes/';
            $batchArray[$counter]['path'] = $ecommerceS.$mailchimpStoreId. $promoR .$promoRuleId. $promoC .$promoCodeId;
    ...
    protected function _getNewPromoCodes($mailchimpStoreId, $magentoStoreId)
    {
        ...
        foreach ($newPromoCodes as $promoCode) {
            ...
            try {
                ...
                if (!$promoRuleSyncData->getId() || $this->isMailchimpSync(
                    $magentoStoreId,
                    $promoRuleSyncData,
                    $helper
                )) {
    ...
    public function makePromoCodesCollection($magentoStoreId)
    {
        ...
        $helper->addResendFilter($collection, $magentoStoreId, $this->isPromoCode());
    ...
    public function joinMailchimpSyncDataWithoutWhere($collection, $mailchimpStoreId)
    {
        ...
        $collection->getSelect()->joinLeft(
            ...
            sprintf($joinCondition, $this->isPromoCode(), $mailchimpStoreId), array(
                'm4m.related_id',
                'm4m.type',
    ...
    protected function getRedemptionUrl($promoCode, $magentoStoreId)
    {
        ...
        $model = Mage::getModel('core/url')->setStore($magentoStoreId)->getUrl(
            $strLoadCoupon,
            array(
            '_nosid' => true,
            '_secure' => true,
    ...
    {
        $helper = $this->getMailChimpHelper();
        $promoCodes = $helper->getAllEcommerceSyncDataItemsPerId($codeId, $this->isPromoCode());
     ...
     * @return string
     */
    protected function isPromoCode()
    {
    ...
    /**
     * @param $magentoStoreId
     * @param $promoRuleSyncData
     * @param $helper
     * @return bool
     */
    protected function isMailchimpSync($magentoStoreId, $promoRuleSyncData, $helper)
    {
        return $promoRuleSyncData->getMailchimpSyncDelta() < $helper->getEcommMinSyncDateFlag($magentoStoreId);
    }
}
